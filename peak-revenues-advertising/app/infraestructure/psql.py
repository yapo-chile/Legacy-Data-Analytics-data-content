...
class Database:
    ...
    def insert_data_direct_sales(self, data_dict: pd.DataFrame) -> None:
        self.log.info('INSERT INTO %s', self.conf.table_direct)
        page_size: int = 10000
        with self.connection.cursor() as cursor:
            psycopg2.extras \
                .execute_values(cursor,
                                """ INSERT INTO """ + self.conf.table_direct +
                                """ ( year,
                                      month,
                                      "monto acumulado"
                                    )
                                    VALUES %s; """, ((
                                        int(row.year),
                                        int(row.month),
                                        int(row.monto_acumulado)
                                    ) for row in data_dict.itertuples()),
                                page_size=page_size)
            self.log.info('INSERT %s COMMIT.', self.conf.table_direct)
            self.connection.commit()
            self.log.info('CLOSE CURSOR %s', self.conf.table_direct)
            cursor.close()

    def insert_data_network_sales(self, data_dict: pd.DataFrame) -> None:
        self.log.info('INSERT INTO %s', self.conf.table_network)
        page_size: int = 10000
        with self.connection.cursor() as cursor:
            psycopg2.extras \
                .execute_values(cursor,
                                """ INSERT INTO """ + self.conf.table_network +
                                """ ( fecha,
                                      month,
                                      "tipo de red",
                                      "monto usd",
                                    ...
                                    VALUES %s; """, ((
                                        ...
                                        str(row.tipo_de_red),
                                        float(row.monto_usd),
                                        float(row.monto_pesos)
                                    ) for row in data_dict.itertuples()),
                                page_size=page_size)
            self.log.info('INSERT %s COMMIT.', self.conf.table_network)
            self.connection.commit()
            self.log.info('CLOSE CURSOR %s', self.conf.table_network)
            cursor.close()
