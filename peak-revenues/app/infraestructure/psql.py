...
class Database:
    ...
    def select_to_dict(self, query) -> pd.DataFrame:
        ...
        self.log.info('Results from query: {}'.format(len(pd_result)))
        cursor.close()
        return pd_result

    def insert_data(self, data_dict: pd.DataFrame) -> None:
        self.log.info('INSERT INTO %s', self.conf.table_csat)
        page_size: int = 10000
        with self.connection.cursor() as cursor:
            psycopg2.extras \
                .execute_values(cursor,
                                """ INSERT INTO """ + self.conf.table_csat +
                                """ ( date_id,
                                      vertical,
                                      revenue_type,
                                      amount
                                ...
                                page_size=page_size)
            self.log.info('INSERT %s COMMIT.', self.conf.table_csat)
            self.connection.commit()
            self.log.info('CLOSE CURSOR %s', self.conf.table_csat)
            cursor.close()
